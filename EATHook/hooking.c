    ...
    uint8_t *module_address, uint32_t module_size)
{
    ...
    if(ki_user_exception_dispatcher == NULL) {
        pipe("WARNING:ntdll!RtlDispa/chException unable to find "
...
PIMAGE_NT_HEADERS GetLocalNtHead(char *lib)
{
	DWORD dwTemp = NULL;
  ...
  HMODULE ImageBase = GetModuleHandle(lib);
...
int EATHook(hook_t *h) {
  ...
  pAddressOfFunctions[index] = ((DWORD)(h -> handler)) - (DWORD)DllBase;
